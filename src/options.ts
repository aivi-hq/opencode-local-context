...
export interface LocalContextOptions {
	/** Base used to resolve a relative contextDir. Defaults to the project's canonical checkout. */
	root?: ContextRoot;
...
function requireNonEmptyString(value: unknown, name: string): string {
	if (typeof value !== "string" || value.length === 0) {
		throw new TypeError(
			`opencode-local-context: ${name} must be a non-empty string`,
		);
	}
	return value;
}
...
function requireFilename(value: unknown, name: string): string {
	...
	if (
		...
		throw new TypeError(
			`opencode-local-context: ${name} must be a filename, not a path`,
		);
	}
	return filename;
}
...
export function resolveOptions(
	...
	if (
		...
		throw new TypeError(
			"opencode-local-context: plugin options must be an object",
		);
	}
	...
	if (root !== "worktree" && root !== "directory") {
		throw new TypeError(
			'opencode-local-context: root must be "worktree" or "directory"',
		);
	}
	...
	if (
		...
		throw new TypeError(
			'opencode-local-context: missingEnv must be "error", "warn", or "empty"',
		);
	}

	const contextDir = requireNonEmptyString(
		input.contextDir ?? DEFAULTS.contextDir,
		"contextDir",
	);
	const generalFile = requireFilename(
		input.generalFile ?? DEFAULTS.generalFile,
		"generalFile",
	);
	...
	if (typeof interpolate !== "boolean") {
		throw new TypeError(
			"opencode-local-context: interpolate must be a boolean",
		);
	}
...
export function resolveContextDirectory(
	...
	const base =
		options.root === "worktree" ? locations.worktree : locations.directory;

...
async function readMarkdown(filename: string): Promise<string | undefined> {
	try {
		...
		throw new Error(`opencode-local-context: failed to read ${filename}`, {
			cause: error,
		});
	}
}
...
export async function loadContextFiles(
	...
	const [prefix, suffix] = agentFilePattern.split("{agent}") as [
		string,
		string,
	];
	...
	try {
		...
		throw new Error(
			`opencode-local-context: failed to scan ${contextDirectory}`,
			{
				cause: error,
			},
		);
